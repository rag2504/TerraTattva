...
import { Heart, ShoppingCart, Menu, X, Plus, Minus, CheckCircle } from "lucide-react";
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center h-20">
          ...
          <div className="flex items-center space-x-3 flex-shrink-0">
            ...
            />
            <Link to="/" className="text-xl sm:text-2xl lg:text-3xl font-bold bg-gradient-to-r from-orange-600 via-red-600 to-orange-700 bg-clip-text text-transparent truncate">
              TerraTattva
            </Link>
          </div>
          ...
          <div className="flex items-center space-x-1 sm:space-x-3 flex-shrink-0 ml-auto lg:ml-0">
              ...
              <DialogContent className="max-w-lg">
                <DialogHeader>
                  <DialogTitle className="flex items-center justify-between text-2xl">
                    <span className="bg-gradient-to-r from-orange-600 to-red-600 bg-clip-text text-transparent">Shopping Cart</span>
                ...
                <div className="max-h-96 overflow-y-auto">
                  {cart.length === 0 ? (
                    <div className="text-center py-12">
                      ...
                      <p className="text-gray-400">Add some beautiful pottery to get started!</p>
                    </div>
                  ) : (
                    <div className="space-y-4">
                      {cart.map((item) => (
                        <div key={item.id} className="flex items-center space-x-4 p-4 border border-orange-100 rounded-xl bg-gradient-to-r from-orange-50 to-red-50">
                          <img 
                            src={item.image} 
                            alt={item.name}
                          ...
                          <div className="flex-1">
                            ...
                            <div className="flex items-center space-x-3 mt-3">
                              ...
                              <span className="text-lg font-bold text-gray-700 min-w-[30px] text-center">{item.quantity}</span>
                              <Button 
                                size="sm" 
                                variant="outline" 
                                className="h-8 w-8 p-0 rounded-full border-orange-300"
                      ...
                      <div className="border-t border-orange-200 pt-6 mt-6">
                        <div className="flex justify-between items-center mb-6">
                          ...
                          <span className="text-3xl font-bold bg-gradient-to-r from-orange-600 to-red-600 bg-clip-text text-transparent">₹{getCartTotal()}</span>
                        </div>
                        <Button 
            ...
            <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
              <SheetTrigger asChild>
                <Button variant="ghost" size="sm" className="lg:hidden p-2 sm:p-3 hover:bg-orange-50 rounded-full">
